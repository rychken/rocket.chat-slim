hideMessagesOfType = []
Sequoia.settings.get /Message_HideType_.+/, (key, value) ->
...
Meteor.methods
	loadHistory: (rid, end, limit=20, ls) ->
		...
		if room.t is 'c' and not Sequoia.authz.hasPermission(fromId, 'preview-c-room') and room.usernames.indexOf(room.username) is -1
			return false
		...
		if end?
			records = Sequoia.models.Messages.findVisibleByRoomIdBeforeTimestampNotContainingTypes(rid, end, hideMessagesOfType, options).fetch()
		else
			records = Sequoia.models.Messages.findVisibleByRoomIdNotContainingTypes(rid, hideMessagesOfType, options).fetch()
		...
		if ls?
			...
			if firstMessage?.ts > ls
				...
				unreadMessages = Sequoia.models.Messages.findVisibleByRoomIdBetweenTimestampsNotContainingTypes(rid, ls, firstMessage.ts, hideMessagesOfType, { limit: 1, sort: { ts: 1 } })
