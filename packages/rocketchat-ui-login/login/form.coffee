Template.loginForm.helpers
	...
	namePlaceholder: ->
		return if Sequoia.settings.get 'Accounts_RequireNameForSignUp' then t('Name') else t('Name_optional')

	showFormLogin: ->
		return Sequoia.settings.get 'Accounts_ShowFormLogin'
	...
	loginTerms: ->
		return Sequoia.settings.get 'Layout_Login_Terms'

	registrationAllowed: ->
		return Sequoia.settings.get('Accounts_RegistrationForm') is 'Public' or Template.instance().validSecretURL?.get()

	linkReplacementText: ->
		return Sequoia.settings.get('Accounts_RegistrationForm_LinkReplacementText')

	passwordResetAllowed: ->
		return Sequoia.settings.get 'Accounts_PasswordReset'

	requirePasswordConfirmation: ->
		return Sequoia.settings.get 'Accounts_RequirePasswordConfirmation'

	emailOrUsernamePlaceholder: ->
		return Sequoia.settings.get('Accounts_EmailOrUsernamePlaceholder') or t("Email_or_username")

	passwordPlaceholder: ->
		return Sequoia.settings.get('Accounts_PasswordPlaceholder') or t("Password")
...
Template.loginForm.events
		...
		if formData
			if instance.state.get() is 'email-verification'
				Meteor.call 'sendConfirmationEmail', s.trim(formData.email), (err, result) ->
					...
					Sequoia.callbacks.run('userConfirmationEmailRequested');
			...
			if instance.state.get() is 'forgot-password'
				Meteor.call 'sendForgotPasswordEmail', s.trim(formData.email), (err, result) ->
					if err
						...
						Sequoia.Button.reset(button)
						Sequoia.callbacks.run('userForgotPasswordEmailRequested');
			...
			if instance.state.get() is 'register'
				...
				Meteor.call 'registerUser', formData, (error, result) ->
					...
					Sequoia.callbacks.run('userRegistered');
				...
				if Sequoia.settings.get('LDAP_Enable')
					loginMethod = 'loginWithLDAP'
				if Sequoia.settings.get('CROWD_Enable')
	...
	'click .register': ->
		...
		Sequoia.callbacks.run('loginPageStateChange', Template.instance().state.get());
...
Template.loginForm.onCreated ->
	...
	Tracker.autorun =>
		Accounts_CustomFields = Sequoia.settings.get('Accounts_CustomFields')
		if typeof Accounts_CustomFields is 'string' and Accounts_CustomFields.trim() isnt ''
			try
				@customFields.set JSON.parse Sequoia.settings.get('Accounts_CustomFields')
			catch e
	...
	@validate = ->
		...
		if instance.state.get() is 'register'
			if Sequoia.settings.get('Accounts_RequireNameForSignUp') and not formObj['name']
				validationObj['name'] = t('Invalid_name')

			if Sequoia.settings.get('Accounts_RequirePasswordConfirmation') and formObj['confirm-pass'] isnt formObj['pass']
...
Template.loginForm.onRendered ->
	...
	Tracker.autorun =>
		Sequoia.callbacks.run('loginPageStateChange', this.state.get())
