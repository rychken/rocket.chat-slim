Sequoia.sendMessage = (user, message, room, upsert = false) ->
	...
	if not room.usernames? || room.usernames.length is 0
		room = Sequoia.models.Rooms.findOneById(room._id)
	...
	if message._id? and upsert
		_id = message._id
		delete message._id
		Sequoia.models.Messages.upsert {_id: _id, 'u._id': message.u._id}, message
		message._id = _id
	else
		message._id = Sequoia.models.Messages.insert message
	...
	Meteor.defer ->
		...
		Sequoia.callbacks.run 'afterSaveMessage', message, room
