Meteor.methods
	addUserToRoom: (data) ->
		...
		if not Sequoia.authz.hasPermission(fromId, 'add-user-to-room', room._id)
			throw new Meteor.Error 'error-not-allowed', 'Not allowed', { method: 'addUserToRoom' }
		...
		newUser = Sequoia.models.Users.findOneByUsername data.username
		Sequoia.addUserToRoom(data.rid, newUser, Meteor.user());
